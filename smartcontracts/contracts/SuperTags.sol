...
contract SuperTags is Context, AccessControlEnumerable, ERC721Enumerable, ERC2981, IERC4907 {
    ...
    bytes32 public constant ST_ADMIN_ROLE = keccak256("ST_ADMIN_ROLE");
    bytes32 public constant ST_OPERATOR_ROLE = keccak256("ST_OPERATOR_ROLE");
    bytes32 public constant ST_CREATOR_ROLE = keccak256("ST_CREATOR_ROLE");
    ...
    constructor(
        ...
        _setRoleAdmin(ST_OPERATOR_ROLE, ST_ADMIN_ROLE);
    }
    ...
    function _transferFunds(
        ...
        require(callSuccess, "SuperTags: Transfer failed");
    ...
    function registerTag(
    ...
    ) public onlyRole(ST_CREATOR_ROLE) returns (uint256) {
        ...
        require(
            ...
            "SuperTags: Royalty can't be more than 10%"
        );
        _setTokenRoyalty(
            currentTokenID,
            _msgSender(),
        ...
        emit SuperTagsAssetCreated(currentTokenID, _msgSender(), metadataURI);
    ...
    function delegateTagRegistration(
        ...
        string memory metadataURI,
        uint96 royaltyPercentBasisPoint
    ) public onlyRole(ST_OPERATOR_ROLE) returns (uint256) {
        // We cannot just use balanceOf to create the new tokenId because tokens
        // can be burned (destroyed), so we need a separate counter.
        Counter++;
        uint256 currentTokenID = Counter;
        ...
        emit SuperTagsAssetCreated(currentTokenID, creator, metadataURI);
        return currentTokenID;
    }
    ...
    function _setTokenURI(
        ...
        require(_exists(tokenId), "SuperTags: Non-Existent Asset");
    ...
    function setUser(uint256 tokenId, address user, uint64 expires) public {
        require(
            _isApprovedOrOwner(_msgSender(), tokenId),
            "SuperTags: Not token owner Or approved"
        );
        require(
            userOf(tokenId) == address(0),
            "SuperTags: item is already subscribed"
        );
    ...
    function rent(uint256 _tokenId, uint256 _timeInHours) external payable {
        require(_exists(_tokenId),
            "SuperTags: Invalide Token Id"
        );
        require(
            rentables[_tokenId].isRentable,
            "SuperTags: Not available for rent"
        );
        require(
            userOf(_tokenId) == address(0),
            "SuperTags: NFT Already Subscribed"
        );
        require(
            _timeInHours > 0,
            "SuperTags: Time can't be less than 1 hour"
        );
        require(
            _timeInHours <= 4320,
            "SuperTags: Time can't be more than 6 months"
        );

        uint256 amount = amountRequired(_tokenId, _timeInHours);

        require(msg.value >= amount, "SuperTags: Insufficient Funds");
